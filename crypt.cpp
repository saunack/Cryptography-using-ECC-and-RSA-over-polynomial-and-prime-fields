...
#define PrimeMin 1000
#define PrimeMax 1000000
#define ECCMin 10
#define ECCMax 30
#define RSAMin 1000000
#define RSAMax 5000000
...
void G2Ecc_key( std::ostream& Y, mpz_class& priv, G2E& g, G2E& publ ){
	g = G2E();
	mpz_class p(0), P, primePol[]={19, 25, 31, 47, 41, 59, 61, 55};					//List of irreducible polynomials
	p = primePol[ random(0,7).get_ui() ];											//Selecting random polynomial from the list
	GF2::set(p);
	P = GF2::get();
...
void GF2_key( std::ostream& Y, mpz_class& priv, GF2& g, GF2& publ ){
	g = GF2();
	mpz_class p(0), P, primePol[]={19, 25, 31, 47, 41, 59, 61, 55};				//List of irreducible polynomials
	p = primePol[ random(0,7).get_ui() ];										//Slecting random polynomial from the list
	GF2::set(p);
	P = GF2::get();
	priv = random(2, P - 1);
	while( !isProper(g) ) g = GF2( random(1, P - 1) );
	publ = exponentiation( g, priv );
	Y << p << std::endl << g << std::endl << publ << std::endl;
}
...
mpz_class GF2_dec( std::istream& in, std::ostream& out, mpz_class& privateKey, GF2& generator, GF2& pKey, GF2& sharedKey ){
	...
	//std::cout << "Key: " << sharedKey << std::endl;
	return buff.x;
}
...
void gf2( std::string M, std::string Y ){
	...
	while( publicKey[0] == GF2() || publicKey[0] == GF2(0) || sharedKey[1] == GF2(0) || Hash == 0){
		std::ofstream out(Y + ".key");
...
void RSA_key( std::ostream& Y, mpz_class priv[], PF& publ, PF shared[] ){
	mpz_class p(0);
	mpz_class q(0);
	while( p == 0 )	p = gPrime(RSAMin, RSAMax);
	while( q == 0 || q == p )	q = gPrime(RSAMin, RSAMax);
...
void RSA_verify( std::istream& in, std::istream& inKey, mpz_class H ){
	...
	PF key;
	for(int i=0;i<3;i++)
        inKey >> key;                                   //3rd input is the corresponding key for verifying the sign
	in >> s;
	PF LHS(H);
